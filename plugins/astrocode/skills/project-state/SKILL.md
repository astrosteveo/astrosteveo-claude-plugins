...
Use the `/commit` workflow (see `astrocode:commit` skill) for all commits. That skill handles conventional commit formatting and the post-commit PROGRESS.md update. The key rule: every source commit is followed by a separate `chore(state): update project state` commit that syncs PROGRESS.md.
