...
Present findings using the audit-report skeleton in `references/report-template.md`. Group by issue category (Contradictions first), quote the offending content with file paths, and give a concrete recommendation per item. If the extended audit was performed, append matching sections for rules, project CLAUDE.md, and global CLAUDE.md conflicts.

#### Step 5: Present the Execution Plan

After the report, present a concrete action plan using the execution-plan skeleton in `references/report-template.md`. Each line is a single verb (DELETE / EDIT) with the target file and a one-clause reason; close with totals and net context reduction.
